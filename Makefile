...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2023/12/12 01:49:43 by tmnatsak          #+#    #+#              #
#    Updated: 2024/01/13 15:02:19 by tmnatsak         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
INCS			= -I./includes -I./readline/include
CC				= cc 
RM				= rm -rf
CFLAGS			= -fsanitize=address -g -Wall -Wextra -Werror
LIBFLAGS		= -Lreadline/lib -lreadline -lhistory
...
readline_install:
				...
				@./readline_config.sh readline > /dev/null 2>&1 & \
				pid=$$!; \
...
$(NAME):		$(OBJS) $(LIBFT)
				...
				@$(PRINTF) "\r%100s\r$(GREEN)✧ $(PURPLE)Minishell $(GREEN)successfully compiled! ✧$(RESET)\n"
...
clean:			clean_lib clean_readline
				@$(RM) $(OBJ_DIR)
				@$(PRINTF) "$(GREEN)☽ $(PURPLE)Minishell $(GREEN)successfully cleaned!$(RESET)\n"

clean_readline:
				@$(RM) -r readline
				@$(PRINTF) "$(GREEN)☽ Readline $(GREEN)successfully cleaned!$(RESET)\n"

clean_lib:
				@$(MAKE) clean -C $(LIBFT_PATH) > /dev/null 2>&1
				@$(PRINTF) "$(GREEN)☽ Libft $(GREEN)successfully cleaned!$(RESET)\n"
